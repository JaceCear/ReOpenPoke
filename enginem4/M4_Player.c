...
void M4_PlaySong(u32 Player, const SongData *Song) {
	...
	for(trackCounter=0;trackCounter < Song->Tracks;trackCounter++, Track++)
	{
		Track->Status    = M4_Active;
		Track->Self      = trackCounter;
		Track->Index     = 0;
		Track->Note      = 0;
		Track->Velo      = 0;
		Track->Timer     = 0;
		...
		Track->MicroTune = 0;
		Track->LastByte  = 0;
		Track->Data      = Song->Part[trackCounter];
	}

	for(; trackCounter < 16; trackCounter++, Track++)
		Track->Status = 0;
...
void M4_PlayByIdx(u32 Idx) {
	struct _STT *SongP = &song_table[Idx];
